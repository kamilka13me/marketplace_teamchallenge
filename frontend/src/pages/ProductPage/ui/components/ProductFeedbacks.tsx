...
import { getRouteProductComments } from '@/shared/const/routes';
...
const ProductFeedbacks: FC<Props> = (props) => {
  const { product, feedbacks, rating, refetchFeedbacks } = props;
  ...
  return (
    <HStack className="bg-dark-grey max-w-[646px] w-full rounded-2xl p-4">
      ...
      {feedbacks.totalComments > 0 ? (
        <>
          <Link
            to={getRouteProductComments(product._id)}
